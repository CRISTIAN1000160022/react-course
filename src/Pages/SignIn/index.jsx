...
function SignIn() {
  ...
  const [view, setView] = useState("user_info");
  const form = useRef();

  //Account from local storage
  const account = localStorage.getItem("account");
  const parsedAccount = JSON.parse(account);
  //Has an account in local storage
  const noAccountInLocalStorage = parsedAccount
    ? Object.keys(parsedAccount).length === 0
    : true;
  // Account from local state
  const noAccountInLocalState = context.account
    ? Object.keys(context.account).length === 0
    : true;
  //Has an account in local state or local storage
  const hasUserAnAccount = !noAccountInLocalStorage || !noAccountInLocalState;

  //Sign in user and redirect to home
  const handleSignIn = () => {
  ...
  //Render Log In view or Create User Info view
  const renderLogIn = () => {
    return (
      <div className="flex flex-col w-80">
        ...
        <button
          ...
          onClick={() => setView("create_user_info")}
          disabled={hasUserAnAccount}
        >
          Sign up
        </button>
  ...
  const renderView = () =>
    view === "create_user_info" ? renderCreateUserInfo() : renderLogIn();
